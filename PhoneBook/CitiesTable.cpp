...
#define ERROR_VERSION_CONTROL_MESSAGE _T("Error in version control - Update Counters does not match. Error: %d, Query: %s.\n")
...
BOOL CCitiesTable::SelectWhereID(const long lID, CITIES& recCity)
{
	...
	hResult = ExecuteCommandGetRequestedRecord(strQuery);
	if (FAILED(hResult))
	{
...
BOOL CCitiesTable::UpdateWhereID(const long lID, const CITIES& recCity)
{
	...
	hResult = ExecuteCommandGetRequestedRecord(strQuery, &oUpdateDBPropSet);
	if (FAILED(hResult))
	{
		TRACE(ERROR_ExecuteCommandGetRequestedRecord_MESSAGE);
	...
	{
		TRACE(ERROR_VERSION_CONTROL_MESSAGE, hResult, strQuery.GetString());
